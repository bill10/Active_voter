...
double Dynamic_Voter::simulate(float alpha, float beta, int mode, float gamma) {
	long int e1,e2;
...
//	dt=(int)population.size();
	int action=-1; //0:voting, 1:rewire, 2:close triangle, 3:close triangle unsuccessful

// create file to save the frequencies of opinions throughout the process
//	filename<<"mode"<<mode<<"n"<<population.size()<<"k"<<(edges.size())*2/(population.size())<<"a"<< setiosflags(ios::fixed) << setprecision(2)<<alpha<<"g"<<gamma<<"u";
...
//		i++;
//		filename.str("");
//		filename<<"mode"<<mode<<"n"<<population.size()<<"k"<<(edges.size())*2/(population.size())<<"a"<<alpha<<"g"<<gamma<<"u";
//		for (j=0; j<(int)sites.size()-1; j++)
//			filename <<(float)sites[j].size()/population.size()<<"_";
...
//		filename <<(float)sites[j].size()/population.size()<<"_";
//	if (i>1)
//		filename<< (float)sites[j].size()/population.size()<<"("<< i <<").component";
//	else
//		filename<< (float)sites[j].size()/population.size()<<".component";
//	ofstream pFile_comp(filename.str().c_str());

//	create file to save the component sizes of 1-nodes throughout the process
//	filename.str("");
//	filename<<"mode"<<mode<<"n"<<population.size()<<"k"<<(edges.size())*2/(population.size())<<"a"<<alpha<<"u";
//	for (j=0; j<(int)sites.size()-1; j++)
//		filename <<(float)sites[j].size()/population.size()<<"_";
//	if (i>1)
//		filename<< (float)sites[j].size()/population.size()<<"("<< i <<").component1";
//	else
//		filename<< (float)sites[j].size()/population.size()<<".component1";
...
//		pFile_process<<"N"<<j<<" ";
//	pFile_process<<"N_neq ave_path transitivity";
...
//	pFile_process<<endl;
//	pFile_process<<step<<" "<<action<<" ";
//	print_statistics_igraph(pFile_process);
...
//			if (random_number.real()<=exp(-(double)edge_boundary.size()/1000)){
...
//			}
	...
	else if (mode==8) {// rewire to random nodes of distance 2
		while (edge_boundary.empty() == false) {
//			real_time+=(float)edges.size()/edge_boundary.size();
			e1 = random_number.integer(edge_boundary.size());
			edge_it=edge_boundary[e1];
			if (random_number.real()<alpha){
				rewire_to_closest(edge_it);}
			else{
				adopt_state(edge_it);}
			step++;
...
//			}
	...
	else if (mode==9) {// rewire to random nodes of distance 2 with probability gamma
		while (edge_boundary.empty() == false) {
//			real_time+=(float)edges.size()/edge_boundary.size();
			e1 = random_number.integer(edge_boundary.size());
			edge_it=edge_boundary[e1];
			if (random_number.real()<alpha) {
				if (random_number.real()<gamma) {
					action=rewire_to_closest(edge_it);}
				else {
					action=1;
					rand_rewire(edge_it); }
			}
...
int Dynamic_Voter::rewire_to_closest(vector<Edge>::iterator edge_it) {
	queue<long int> que;
	vector<bool> visited;
	vector<long int> candidates;
	vector<Node>::iterator person1_it, person2_it;
	long int i1, i2, n1, n2;
	bool need_new_neigh=true;
	vector<Edge>::iterator edge_it2;
	list<vector<Edge>::iterator>::iterator neigh_edge_it;
	int current_size, new_size, i, j, distance=2;
	int action;

	visited.assign(population.size(),0);
	...
	for(i=0;i<distance;i++) {
		...
		for (j=0;j<current_size;j++) {
			...
			for (neigh_edge_it = population[n1].edge_list.begin(); neigh_edge_it!=population[n1].edge_list.end(); neigh_edge_it++) {
				edge_it2 = *neigh_edge_it;
				n2=edge_it2->person1->ID;
				if(n2==n1)
					n2=edge_it2->person2->ID;
				if (visited[n2]==0) {
					que.push(n2);
					visited[n2]=1;
					new_size++; }}}
	...
	while(!que.empty()) {
		n1=que.front();
		que.pop();
		if (!is_neighbor(i1, n1))
			candidates.push_back(n1); }

	if (!candidates.empty()) {
		i2 = candidates[random_number.integer(candidates.size())];
		action=2; }
	else {
		action=3;
		while (need_new_neigh) {
...
void Dynamic_Voter::print_statistics_igraph(ofstream &pFile_process) {
	...
	for (j=0; j<(int)sites.size(); j++)
		pFile_process<<sites[j].size()<<" ";
	pFile_process<<edge_boundary.size()<<" "<<(float)avg_path<<" "<<(float)clustering<<endl;
