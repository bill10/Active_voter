...
int main(int argc, char* argv[]){ // -n number_of_nodes -m number_of_edges -d degree -u number_of_opinions initial_densities -a alpha -r mode -b beta -g gamma -p rewiring_watts_strogatz
...
//	test if the file already exists
	...
	while (testfile.is_open()){
		i++;
		filename.str("");
		filename<<"mode"<<mode<<"n"<<number_of_nodes<<"k"<<degree<<"a"<<alpha<<"g"<<gamma<<"u";
		for (j=0; j<number_of_opinions-1; j++)
			filename <<initial_density[j] <<"_";
...
//	create file to save node opinions in the end
	...
	for(realization=1;realization<=1;realization++){
		...
		steps = dv->simulate(alpha,beta,mode,gamma,process,"","","","");
		dv->component();
		outfile <<realization<<" "<< alpha <<" "<<gamma<<" ";
		for (i=0; i<number_of_opinions; i++)
